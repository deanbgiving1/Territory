...
q_learning_ai.load_q_table('territory/q_table.pkl')
    ...
    while running:
        ...
        if game.turn.startswith("white"):  # Human player's turn
            ...
            if game.initial_phase:
                ...
                state = q_learning_ai.get_state()
                move, exploration = q_learning_ai.choose_action(state, q_learning_ai.find_valid_moves())
                if move:
                    q_learning_ai.perform_action(move)
                    q_learning_ai.log_action(move, 'in_progress', exploration)
                game.advance_turn()
